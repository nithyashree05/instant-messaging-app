...
def connect_database():
    if emailEntry.get()=='' or usernameEntry.get()=='' or passwordEntry.get()=='' or confirmEntry.get()=='':
        ...
        try:
            ...
            query='use user_data'
            cur.execute(query)
            query=('create table data(id int auto_increment primary key not null,email varchar(50),username varchar(100),password varchar(40),timestamp TIMESTAMP;')
            cur.execute(query)
            
        except:
            cur.execute('use user_data')
            cur.execute('create table data(id int auto_increment primary key not null,email varchar(50),username varchar(100),password varchar(40),timestam TIMESTAMP')
...
background=PhotoImage(file=r'C:\python instant messaging\signup.png')
